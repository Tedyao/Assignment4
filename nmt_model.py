...
class NMT(nn.Module):
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1, dim=0):
            ...
            new_dec_state, new_o, e_t = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(new_o)
            o_prev = new_o
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = enc_hiddens.permute([0,2,1]).bmm(alpha_t.unsqueeze(dim=2)).squeeze(dim=2)
        U_t = torch.cat((dec_hidden, a_t), dim=1)
        V_t =self.combined_output_projection(U_t)
        O_t = self.dropout(self.tanh(V_t))
